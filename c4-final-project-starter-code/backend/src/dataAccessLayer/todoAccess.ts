...
export class ToDoAccess {
    constructor(
        ...
        private readonly todoListTable = process.env.TODOLIST_TABLE,
        private readonly imageIdIndex = process.env.TODO_ID_INDEX,
    ...
    async createTodo(todoItem: TodoItem): Promise<TodoItem> {
        logger.info('Creating new to do item.');
        logger.info(this.todoListTable);
    ...
    async getToDoForUser(userId: string): Promise<TodoItem[]> {
        logger.info('getToDoForUser', 'userId:', userId, 'table',this.todoListTable);
        ...
        logger.info('Todo list for a user', result.Items);
